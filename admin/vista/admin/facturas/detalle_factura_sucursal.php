...
<body>
	...
	<div>
		...
		<!--CREAR SUCURSAL -->
		<table id="tablaDatos">
			<caption>El detalle de su factura sucursal</caption>
			<tr>
                <th>Cantidad</th>
                <th>Imagen</th>
                <th>Producto</th>
            ...
            $cabecera = $_GET['codigoCabecera'];


			include '../../../../config/conexionBD.php';

			$sql = "SELECT * FROM factura_detalle WHERE fadet_eliminada = 'N' AND fadet_cod_facab = $cabecera";
			$result = $conn->query($sql);

			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
                    $codPro = $row['fadet_cod_pro'];
                    ...
                    if($result2 != null){
                        $row2 = $result2->fetch_assoc();
                            ...
                            echo "<tr>";
                            echo "   <td>" . $row['fadet_cantidad'] . "</td>";
                            echo "   <td> <img src='data:image/jpg;base64," . base64_encode($row2['pro_imagen']) . "' alt='imagenPerfil' width='100' height='100'></td>";
                            echo "   <td>" . $row2['pro_descripcion'] . "</td>";
                            echo "   <td>" . $row['fadet_subtotal']. "</td>";
                            echo "   <td>" . $row['fadet_iva'] ."</td>";
                            echo "   <td>" . $row['fadet_total'] ."</td>";                        
                        
                    }else {
                        echo "<tr>";
                        echo "   <td colspan='7'> No Existen FACTURAS </td>";
                        echo "</tr>";
                    }	
				}
			} else {
