...
<body>
	...
	<div class="header">
	    ...
	    <div>
                        ...
                        $sql= "SELECT * FROM sucursal WHERE suc_codigo = '$codigo' AND suc_eliminada = 'N'";
                            $result= $conn->query($sql);
                            if($result->num_rows> 0) {
                                while($row= $result->fetch_assoc()) {
                        ?>
			<!--MIS DATOS -->
			    <div class="login-page">
                    <div class="container"> 
                        <h3 class="w3ls-title w3ls-title1">Eliminación de Datos de la Sucursal</h3>  
                        <div class="login-body">
                            <!--DATOS DEL USUARIO =========================== -->
                            <form  method="POST" action="../../../controladores/admin/sucursal/eliminar_sucursal.php">
                                
                            <input type="hidden" id="codigo" name="codigo" value="<?php echo $codigo ?>" />      

                                <label for="cedula">Nombre</label>
                                <input type="text" id="nombre" class="user" name="nombre" value="<?php echo $row['suc_nombre'] ?>">

                                <label for="cedula">Dirección</label>
								<input type="text" id="direccion" class="user" name="direccion" value="<?php echo $row['suc_direccion'] ?>" >
								
								<label for="ciudad">Ciudad</label>
                                <input type="text" id="ciudad" class="user" name="ciudad" value="<?php echo $row['suc_ciudad'] ?>" >

								<input type="submit" id="eliminar" name="eliminar" value="Eliminar Datos">
